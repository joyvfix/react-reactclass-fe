...
const UpdatePage = () => {
  // get params
  const { id } = useParams();
  const menuData = foodMenu.find((menu) => menu.title == id);

  const [title, setTitle] = useState(menuData.title);
  const [price, setPrice] = useState(menuData.price);
  const [coverImage, setCoverImage] = useState(menuData.cover);
  const [coverUrl, setCoverUrl] = useState(menuData.cover);
      ...
      <form onSubmit={handleSubmit}>
        <div className="flex w-1/2 flex-col gap-5">
          ...
          <div className="flex gap-2">
            <Button className={"w-[20%] py-3 bg-red-500 hover:bg-red-600 "}>
              Delete
            </Button>
            <Button className={"w-[20%] py-3"}>Update</Button>
          </div>
        </div>
      </form>
    </div>
